 ...
 *
 * The followings are the available columns in table 'check_code':
 * @property integer $id
...
class CheckCode extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, check_code, time', 'required'),
			array('user_id, time', 'numerical', 'integerOnly'=>true),
			array('check_code', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, check_code, time', 'safe', 'on'=>'search'),
		);
	}
